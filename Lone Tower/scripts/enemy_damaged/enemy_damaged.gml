...
if log_name = "Skeleton" {instance_create_depth(x,y,depth-1,skeleton_death) store.skeletons_slain +=1 if (store.challenge_a=1 or store.challenge_b=1 or store.challenge_c=1) and store.goal_skeleton<75+(25*store.lvl_skeleton) {store.goal_skeleton+=1 if store.goal_skeleton>=75+(25*store.lvl_skeleton) {store.challenge_flag=1}}}
...
if log_name = "Dragon" {instance_create_depth(x,y,depth-1,dragon_death) store.dragons_slain +=1 if (store.challenge_a=0 or store.challenge_b=0 or store.challenge_c=0) and store.goal_dragon<1+(store.lvl_dragon) {store.goal_dragon+=1 if store.goal_dragon>=1+(store.lvl_dragon) {store.challenge_flag=1}}}
...
if log_name = "Bee" {instance_create_depth(x,y,depth-1,bee_death) store.bees_slain +=1 if (store.challenge_a=12 or store.challenge_b=12 or store.challenge_c=12) and store.goal_pest<20+(5*store.lvl_pest) {store.goal_pest+=1 if store.goal_pest>=20+(5*store.lvl_pest) {store.challenge_flag=1}}}
